...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"2px solid #dbd9f4","padding":"10px","margin":"10px 0 0 0","alignItems":"center","borderRadius":"8px","flexWrap":"wrap","background":"#f8f8fc","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>反馈类型</div>
        <el-select v-model="formSearch.fankuileixing" placeholder="请选择反馈类型" :clearable="true">
          <el-option v-for="(item, index) in fankuileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>反馈标题</div>
        <el-input v-model="formSearch.fankuibiaoti" placeholder="反馈标题" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('xuqiufankui','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#666","borderRadius":"6px","background":"linear-gradient(90deg, rgba(255,233,100,1) 0%, rgba(194,248,126,1) 29%, rgba(181,233,252,1) 61%, rgba(246,172,218,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/xuqiufankuiAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/fankuileixing/fankuileixing').then(res => {
        if (res.data.code == 0) {
          this.fankuileixingOptions = res.data.data;
        }
      });
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.fankuileixing != '') searchWhere.fankuileixing = this.formSearch.fankuileixing;
        if (this.formSearch.fankuibiaoti != '') searchWhere.fankuibiaoti = '%' + this.formSearch.fankuibiaoti + '%';
        this.$http.get('xuqiufankui/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/xuqiufankuiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
