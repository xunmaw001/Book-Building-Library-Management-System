...
<div :style='{"border":"2px solid #dbd9f4","padding":"20px","margin":"10px auto","borderRadius":"8px","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"0"}' label="入库时间" prop="rukushijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"0"}' label="借阅编号" prop="jieyuebianhao">
            <el-select  @change="jieyuebianhaoChange" v-model="ruleForm.jieyuebianhao" placeholder="请选择借阅编号">
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"0"}' label="归还数量" prop="shuliang">
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/tushujieyue/jieyuebianhao', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.jieyuebianhaoOptions = res.data.data;
          }
        });
      },
      // 下二随
      jieyuebianhaoChange () {
        this.$http.get('follow/tushujieyue/jieyuebianhao?columnValue=' + this.ruleForm.jieyuebianhao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.yonghuzhanghao){
              this.ruleForm.yonghuzhanghao = res.data.data.yonghuzhanghao
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('guihairuku/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post(table+`/update`,obj).then(res => {});
                          this.$http.post('guihairuku/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
